...
namespace Auralyte
{
    ...
    {
        public static TextureDictionary textureDictionary => textureDictionaryHR;
        ...
        public static void InitialiseAll() {
            AddExtraTextures(textureDictionaryLR, textureDictionaryHR);
            AddExtraTextures(textureDictionaryGSLR, textureDictionaryGSHR);
        }
        ...
        public static void TryEmptyAll() {
        ...
        private void LoadImage(int iconSlot, string path, bool overwrite) => LoadTextureWrap(iconSlot, overwrite, true, () => Auralyte.PluginInterface.UiBuilder.LoadImage(path));
        ...
        public static bool IconExists(uint icon) =>
            Auralyte.DataManager.FileExists(GetIconPath(icon, "", false))
            || Auralyte.DataManager.FileExists(GetIconPath(icon, "en/", false));

        private static TexFile GetIconTex(uint icon, bool hr) =>
            GetTex(GetIconPath(icon, string.Empty, hr))
            ?? GetTex(GetIconPath(icon, Auralyte.DataManager.Language, hr));
        ...
        private static TexFile GetTex(string path)
        {
            ...
            {
                if (path[0] == '/' || path[1] == ':')
                    tex = Auralyte.DataManager.GameData.GetFileFromDisk<TexFile>(path);
            }
            catch { }

            tex ??= Auralyte.DataManager.GetFile<TexFile>(path);
            return tex;
        }
        ...
        private TextureWrap LoadTextureWrapSquare(TexFile tex)
        {
            ...
            if (tex.Header.Width > tex.Header.Height)
            {
                ...
                return Auralyte.PluginInterface.UiBuilder.LoadImageRaw(newData, tex.Header.Width, tex.Header.Width, 4);
            }
            else if (tex.Header.Width < tex.Header.Height)
            {
                ...
                return Auralyte.PluginInterface.UiBuilder.LoadImageRaw(newData, tex.Header.Height, tex.Header.Height, 4);
            }
            else
            {
                return Auralyte.PluginInterface.UiBuilder.LoadImageRaw(imageData, tex.Header.Width, tex.Header.Height, 4);
            }
        }
